...
class ScholarQuerier {
    constructor() {
        ...
        this.GET_SETTINGS_URL = ScholarConf.SCHOLAR_SITE + '/scholar_settings?sciifh=1&hl=en&as_sdt=0,5';
        ...
        this.SET_SETTINGS_URL = ScholarConf.SCHOLAR_SITE + '/scholar_setprefs?' +
            'q=' +
    ...
    apply_settings(settings) {
        ...
        let html = this.get_HTTPresponse(
            ...
            logMsg = 'dump of settings form HTML',
            errMsg = 'requesting settings failed'
        );
        ...
        let soup = ScholarSoup.make_soup(html);
        ...
        if (settings.citeForm != 0) {
            urlArgs['scis'] = 'yes';
            urlArgs['scisf'] = '&scisf=' + settings.citeForm;
        }

        html = this.get_HTTPresponse(
            url = this.SET_SETTINGS_URL,
            logMsg = 'dump of settings result HTML',
            errMsg = 'applying settings failed'
        );
    ...
    send_query(query) {
        ...
        let html = this.get_HTTPresponse(
            url = query.get_url(),
            logMsg = 'dump of query response HTML',
            errMsg = 'results retrieval failed'
        );
     ...
     */
    save_cookies() {
        if (ScholarConf.COOKIES == undefined) {
            return false;
        }
        ...
        try {
            ...
            ScholarUtils('warn', 'could not save cookies file: ' + e);
            return false;
        }
    }
    ...
    get_HTTPresponse(url, logMsg = undefined, errMsg = undefined) {
        ...
        try {
            ScholarUtils.log('info', 'requesting ' + decodeURIComponent(url));
            let options = {
                url: url,
                headers: {
