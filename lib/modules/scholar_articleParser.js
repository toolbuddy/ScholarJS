...
class ScholarArticleParser {
    ...
    parse(url) {
        ...
        read(url, function(err, article, res) {
            ...
            for (let div in self.soup.findAll(self._tag_resultsChecker)) {
     ...
     */
    _parse_globals() {
        let tag = this.soup.find('div', { id: 'gs_ab_md' });
    ...
    static _tag_resultsChecker(tag) {
        return tag.attrs['name'] === 'div' && ScholarArticleParser._tag_hasClass(tag, 'gs_r');
    }
