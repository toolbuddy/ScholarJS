...
Scholar.engine = function(options) {
    let conf = new ScholarConf();
    let utils = new ScholarUtils();
    ...
    let querier = new ScholarQuerier();
    let settings = new ScholarSettings();

    if (options.citations == 'bt') {
        settings.set_citeFormat(ScholarSettings.CITATION_BIBTEX);
    } else if (options.citations == 'en') {
        settings.set_citeFormat(ScholarSettings.CITATION_ENDNOTE);
    } else if (options.citations == 'rm') {
        settings.set_citeFormat(ScholarSettings.CITATION_REFMAN);
    } else if (options.citations == 'rw') {
        settings.set_citeFormat(ScholarSettings.CITATION_REFWORKS);
    ...
    if (options.clusterId) {
        ...
        if (options.public) {
            query.set_publication(options.publication);
        }
